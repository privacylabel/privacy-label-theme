...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<?php print template_url(); ?>../../../aanpassingen.css" />
    <script>
    ...
    <script>
        $(document).ready(function () {
            ...
            // VIDEO PLAYER
            $('#btn-btn-20211125133208').click(function(){
                console.log("should show video");
                if( $('#video-overlay').length == 0){
                    $('body').append('<div id="video-overlay"><div><video id="main-vid" src="/videos/privacylabel_in_a_nutshell_x624.mp4" controls="" playsinline width="624" height="auto">Sorry, your browser cannot play this video.</video></div></div>');
                    $('#main-vid')[0].play();
                    $('#video-overlay').click(function(event){
                        console.log(event);
                        console.log(event.target.id);
                        console.log(event.currentTarget);
                        if(event.target.id != "main-vid"){
                            $(this).fadeOut();
                            $('#main-vid')[0].pause();
                        }
            ...
            $('#element_row_1631895862327').click(function(event){
                if(event.target.classList[0] == 'background-image-holder'){
                    location.replace("./privacy-label-examples")
                }
